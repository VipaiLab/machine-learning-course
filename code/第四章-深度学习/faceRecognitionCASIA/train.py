...
def train():
    ...
    for e in range(conf.epoch):
        for data, labels in tqdm(dataloader, desc="Epoch "+str(e)+"/"+str(conf.epoch),ascii=True, total=len(dataloader)):
        ...
        print("Epoch "+str(e)+"/"+str(conf.epoch)+", Loss: }"+str(loss))
        backbone_path = checkpoints+"/"+str(e)+".pth"
